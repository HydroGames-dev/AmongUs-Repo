...
class Game{
	...
	public function endEmergencyTime() : void{
		...
		if($duplicate){
			...
			if($character === null){
				AmongUs::getInstance()->getScheduler()->scheduleRepeatingTask(new DisplayTextTask($this, "No one ejected (skipped)", count($this->filterImposters()) . " impostors remain"), 3);
			}else{
				AmongUs::getInstance()->getScheduler()->scheduleRepeatingTask(new DisplayTextTask($this, "{$topVote} was " . ($character instanceof Imposter ? "the" : "not the") . " impostor", count($this->filterImposters()) . " remain"), 3);
